...
public class Service {
    ...
    @Autowired
    public Service(AccountRepo accountRepo, ICategoryRepo categoryRepo, IProductImagesRepo productImagesRepo,
                   IProductRepo productRepo, IPaymentTypeRepo paymentTypeRepo, IPaymentMethodRepo paymentMethodRepo,
                   IAddressRepo addressRepo, ICartItemRepo cartItemRepo, JwtUtil jwtUtil, JdbcTemplate jdbcTemplate,
...
//    }
    ...
    public ResponseObject savePaymentMethod(JSONObject json) {
        ...
        try {
            paymentMethod = paymentMethod.constructEntity(json.getInt("accountId"), json.getInt("addressId"),
                    json.getInt("typeId"), json.getString("nameOnCard"), json.getString("cardNumber"),
                    json.getString("exp"), json.getString("cvv"));
            if(json.has("pmid")) paymentMethod.setPmId(json.getInt("pmid"));
            if(json.has("favorite")) paymentMethod.setFavorite(json.getBoolean("favorite"));
    ...
    public ResponseObject getAllPaymentMethods(int accountId) {
        ResponseObject response = new ResponseObject();
        String query =
                "SELECT pt.imageSrc, pt.TypeName, pm.NameOnCard, pm.CardNumber, pm.Cvv, pm.Exp, pm.Favorite, pm.PMID, pm.AddressId " +
                        "FROM PaymentType pt " + "INNER JOIN PaymentMethod PM on pt.TypeId = PM.TypeId " +
                        "where AccountID = " + accountId + " order by pm.Favorite DESC";
    ...
    public ResponseObject setPaymentMethodFavorite(JSONObject jsonPayload) {
        ResponseObject response = new ResponseObject();
        String query = "update PaymentMethod set Favorite = 0 where AccountID = " + jsonPayload.getInt("accountId") +
                " and Favorite = 1;" + " update PaymentMethod set Favorite = 1 where AccountID = " +
                jsonPayload.getInt("accountId") + " and PMID =  " + jsonPayload.getInt("PMID");
    ...
    public ResponseObject searchProducts(String k, int c, boolean prime) {
        ...
        for(String word : list) {
            query.append(" (Name like '%").append(word).append("%' or Description like '%").append(word).append("%' ")
                    .append("or");
        }
    ...
    public ResponseObject saveProduct(JSONObject payload) {
        ...
        product = product.constructEntity(payload.getString("name"), payload.getString("description"),
                payload.getString("seller"), payload.getDouble("price"), payload.getBoolean("primeEligible"),
                payload.getInt("stock"), payload.getInt("category"));
        product = productRepo.save(product);
        String[] imageList = payload.getString("image").split(",");
        for(String image : imageList) {
            ProductImages productImages = new ProductImages();
            productImages = productImages.constructEntity(product.getProductId(), image);
    ...
    public ResponseObject saveAddress(JSONObject json) {
        ...
        try {
            address =
                    address.constructEntity(json.getInt("accountId"), json.getString("address"), json.getString("city"),
                            json.getString("state"), json.getInt("zipcode"), json.getString("name"));
            if(json.has("addressId")) address.setAddressId(json.getInt("addressId"));
            if(json.has("favorite")) address.setFavorite(json.getBoolean("favorite"));
    ...
    public ResponseObject setAddressFavorite(JSONObject jsonPayload) {
        ResponseObject response = new ResponseObject();
        String query = "update Address set Favorite = 0 where AccountID = " + jsonPayload.getInt("accountId") +
                " and Favorite = 1;" + " update Address set Favorite = 1 where AccountID = " +
                jsonPayload.getInt("accountId") + " and AddressId =  " + jsonPayload.getInt("AddressId");
        jdbcTemplate.update(query);
        response.setStatus(200);
        return response;
    }
    ...
    public ResponseObject fetchCartItems(int accountId) {
        ResponseObject response = new ResponseObject();
        String query =
                "SELECT cart.CartItemId, cart.Price, cart.Quantity, p.Stock, p.PrimeEligible, p.Description, p.Name, p.Seller, x.Image, p.PID " +
                        "FROM CartItem cart " + "    INNER JOIN Product P " +
                        ...
                        "            on cart.PID = x.PID " + "WHERE AccountID=" + accountId + ";";
    ...
    public ResponseObject addToCart(JSONObject jsonPayload) {
        ...
        cartItem = cartItem.constructEntity(jsonPayload.getInt("accountId"), jsonPayload.getDouble("price"),
                jsonPayload.getInt("quantity"), jsonPayload.getInt("productId"));
