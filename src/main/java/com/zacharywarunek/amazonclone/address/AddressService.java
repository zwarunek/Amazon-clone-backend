...
public class AddressService {
    ...
    public ResponseEntity<Object> createAddress(Long account_id, Address address) {
        if(address.getAddress() == null || address.getCity() == null || address.getState() == null ||
                address.getZipcode() == null || address.getFavorite() == null || address.getFirst_name() == null ||
    ...
    @Transactional
    public ResponseEntity<Object> updateAddress(Long account_id, Long address_id, Address addressDetails) {
        Account account = getAccountById(account_id);
        Optional<Address> optionalAddress = addressRepo.findById(address_id);
        if(!optionalAddress.isPresent())
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Address with id " + address_id + " not found");
        Address address = optionalAddress.get();
        if(address.getAccount().getId().equals(account.getId()))
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                                              "Address with id " + address_id + " does not " +
                                                      "belong to account with id " + account_id);
        ...
        if(addressDetails.getFirst_name() != null) address.setFirst_name(addressDetails.getFirst_name());
    ...
    private Account getAccountById(Long account_id) {
        ...
        if(!optionalAccount.isPresent())
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Account with id " + account_id + " not found");
        return optionalAccount.get();
    }

    public ResponseEntity<Address> getFavorite(Long account_id) {
        Optional<Address> optionalAddress = addressRepo.findFavoriteAddressByAccount(getAccountById(account_id));
        if(!optionalAddress.isPresent())
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Account with id " + account_id + " has no " +
                    "favorite address");
