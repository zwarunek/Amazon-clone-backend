...
class AccountControllerTest {
    ...
    void getAllAccounts() throws Exception {
        ...
        given(accountRepo.findAll())
                .willReturn(Arrays.asList(account1, account2));
    ...
    void updateAccount() throws Exception {
        ...
        if (accountDetails.containsKey("password"))
            assertTrue(passwordEncoder.matches(accountDetails.get("password").toString(), accountCaptor.getValue().getPassword()));
        if (accountDetails.containsKey("role"))
            assertNotEquals(accountCaptor.getValue().getRole(), accountDetails.get("role"));


        assertEquals(
                "Updated Account",
                jsonStringToMap(response.getContentAsString()).get("message").asText());
    ...
    void updateAccountUsernameAlreadyExists() throws Exception {
        ...
        accountDetails.put("username", "changedEmail");
        given(accountRepo.findById(1)).willReturn(java.util.Optional.of(account));
        given(accountRepo.findAccountByUsername(accountDetails.get("username").toString())).willReturn(java.util.Optional.of(account));
    ...
    void deleteAccount() throws Exception {
        ...
        MockHttpServletResponse response = mvc.perform(
                        delete("/api/v1/account/" + account.getId()).contentType(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();
    ...
    void deleteAccountNotFound() throws Exception {
        ...
        MockHttpServletResponse response = mvc.perform(
                        delete("/api/v1/account/" + 1).contentType(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();
